...
A continuous, resumable crawler fetches per-ticker fundamentals through yfinance under a hard, configurable request budget (default 330 upstream requests per rolling hour — a deliberately conservative crawl rate chosen because Yahoo aggressively rate-limits scrapers: 429s and YFRateLimitError are endemic and treated by yfinance maintainers as Yahoo policy [E105][E106][E107][E108]). Every upstream call counts against the budget: one symbol sweep costs ~7 requests (3 statement types × 2 frequencies + profile/price). Honest throughput: ≈1,100 symbols/day → the Europe tier cycles well inside a quarter while the worldwide tail refreshes best-effort (ADR-0004 states the full arithmetic). Daily prices for the whole universe deliberately do NOT ride this budget — bulk EOD prices come from the keyless Stooq path (FR-011); Yahoo prices are fetched only for priority symbols. A priority queue orders work Europe → US large caps → rest of world; statement revisits are freshness-driven (quarterly). Raw responses land as versioned Parquet (append-only raw layer). All sources sit behind one Provider interface; a keyed plugin with no key disables itself cleanly (zero-key contract). If Yahoo blocks for an extended period, the crawler parks politely and retries with capped backoff; the API keeps serving the last snapshot with staleness visible (degradation path). [E105][E106][E107][E108][E52][E36][E82]
...
Stooq bulk daily prices: Stooq's keyless CSV endpoints provide EOD prices outside the Yahoo request budget — the primary bulk price path for valuation ratios, and the fallback when Yahoo throttles or hangs (a real, recurring failure mode [E36][E82][E105]). Switch rule: after 3 consecutive rate-limited Yahoo price attempts or a 30-second hang, the price refresher switches to Stooq for the remainder of the cycle. Bars carry provider='stooq' provenance. [E36][E82][E105]

**Acceptance criteria:**
- **Given** Yahoo price requests hit 3 consecutive rate-limit errors or a 30-second hang **When** the price refresher runs **Then** it switches to Stooq for the remainder of the cycle, updates OHLCV bars for Stooq-covered tickers with provider='stooq' provenance, and the dependent valuation ratios recompute on the next compute cycle
- **Given** a ticker unknown to Stooq **When** the Stooq path attempts it **Then** the ticker is skipped with a single log line and remains scheduled for Yahoo on the next cycle — the fallback never blocks the queue
