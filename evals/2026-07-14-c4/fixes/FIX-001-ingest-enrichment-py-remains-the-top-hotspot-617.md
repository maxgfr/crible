# FIX-001 — ingest/enrichment.py remains the top hotspot (617 LOC, 7 run_* cycles in one module) — c2 F1 / c3 F5 opportunity still open  (P2 · OPPORTUNITY · impact med · effort M)

**Opportunity F3:** The audited enrichment cycles all still live in one module: enrichment.py is 617 LOC holding 7 parallel run_* cycles (ESEF enrichment.py:43, EDGAR :131, EDGAR-bulk :214, FSDS :301, Companies House :396, EDINET :437, ESEF-sweep :513). The per-cycle contracts are already clean, so each could move beside its provider (or an ingest/cycles/ package) to stop one file growing with every new source.
**Evidence:** `src/crible/ingest/enrichment.py:43`, `src/crible/ingest/enrichment.py:513`
**Why it matters:** The audited enrichment cycles all still live in one module: enrichment.py is 617 LOC holding 7 parallel run_* cycles (ESEF enrichment.py:43, EDGAR :131, EDGAR-bulk :214, FSDS :301, Companies House :396, EDINET :437, ESEF-sweep :513). The per-cycle contracts are already clean, so each could move beside its provider (or an ingest/cycles/ package) to stop one file growing with every new source.
